...
# --------- Paths (repo-first, then Colab/Drive) ----------
BASE_DIR   = os.path.dirname(__file__)
REPO_WEI   = os.path.join(BASE_DIR, "weights")
...
CSS_PATH = os.path.join(BASE_DIR, "theme.css")
CSS = open(CSS_PATH).read() if os.path.exists(CSS_PATH) else None

# --------- Save report (to Drive path; если Drive не смонтирован — создастся локально) ----------
...
# --------- Models ----------
# 1) YOLO for car crop (COCO, скачивается автоматически)
crop_yolo = None
try:
...
# 2) YOLO for damages — две модели
damage_yolo1 = None     # damage.pt / damage_rust.pt
damage_yolo2 = None     # damage_v2.pt
damage_names1 = None

# сначала ищем веса в репозитории, затем в /content и в Drive
p1 = _first_existing(
    os.path.join(REPO_WEI, "damage.pt"),
    os.path.join(REPO_WEI, "damage_rust.pt"),
    "/content/weights/damage.pt",
    "/content/weights/damage_rust.pt",
    os.path.join(DRIVE_WEI, "damage.pt"),
    os.path.join(DRIVE_WEI, "damage_rust.pt"),
)
p2 = _first_existing(
    os.path.join(REPO_WEI, "damage_v2.pt"),
    "/content/weights/damage_v2.pt",
    os.path.join(DRIVE_WEI, "damage_v2.pt"),
)
